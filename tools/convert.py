...
class Converter(object):
    ...
    def k210_dataset_gen(self):
        ...
        for filename in image_files_list[:num_imgs]:
            image = cv2.imread(filename)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image = np.array(cv2.resize(image, (self._img_size[0], self._img_size[1])), dtype=np.float32)
    ...
    def convert_k210(self, model_path):
        ...
        print(output_path)
        cmd = '{} compile "{}" "{}" -i tflite --dump-ir --dataset-format raw --dataset "{}"'.format(k210_converter_path, model_path, output_path, folder_name)
        print(cmd)
        result = run_command(cmd)
